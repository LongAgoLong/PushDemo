...
public class DeleteTokenApi extends BaseApiAgent {
    private static final String TAG = DeleteTokenApi.class.getSimpleName();
    /**
     * 待删除的push token
     */
    ...
    void onDeleteTokenResult(int rstCode) {
        PushLog.i(TAG, "deleteToken:callback=" + StrUtils.objDesc(handler) + " retCode=" + rstCode);
        if (handler != null) {
    ...
    public void deleteToken(String token, DeleteTokenHandler handler) {
        PushLog.i(TAG, "deleteToken:token:" + StrUtils.objDesc(token) + " handler=" + StrUtils.objDesc(handler));
        this.token = token;
